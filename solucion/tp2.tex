...
\section{Módulo Juego}
  ...
  \begin{algorithm}[H]{\textbf{iIniciar}(\In{m}{mapa}, \In{pjs}{conj(jugador)}, \In{eventosFan}{vector(evento)}) $\to$ $res$ : estr}
        ...
        \State $iIniciarJugadores(res,\ m,\ pjs)$ \Comment $\Theta(\#pjs*DEFINIR + locJugadores)$
        \\
  ...
  \begin{algorithm}[H]{\textbf{iIniciarJugadores}(\Inout{j}{estr}, \In{m}{mapa}, \In{pjs}{conj(jugador)})} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State $\textbf{for} \ (pj,\ localizacion : \ localPJs) \ \ \textbf{do}$                                            \Comment $\Theta(\#pjs*DEFINIR)$
      ...
      \State \tab $infoPtr \gets \& Definir(j.infoJugadores,\ pj,\ info)$                                                 \Comment $\Theta(DEFINIR)$
      \\
      ...
      \medskip
      \Statex \underline{Complejidad:} $\Theta(?)$
      \Statex \underline{Justificación:} Copiar y generar iteradores, tuplas y conjuntos es $\Theta(1)$. Definir es $\Theta(DEFINIR)$. Luego, definir \#pjs es $\Theta(\#pjs*DEFINIR)$. Finalmente, la complejidad de todo el algoritmo es $\Theta(\#pjs*DEFINIR + locJugadores)$.
    \end{algorithmic}
  \end{algorithm}
