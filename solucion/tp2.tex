...
\section{Módulo Juego}
  ...
  \begin{Interfaz}
    ...
    [$\BigO(|pj| + \#infoActualFanVivos(j) * tam(habitacion(j)) + \#infoActualPjVivos(j))$]
    [actualiza con la acción $a$ del jugador $pj$.]

    \InterfazFuncion{ejecutarSinAccion}{\Inout{j}{juego}}{}%REVISAR
    {$j \igobs pasar(j_0)$}%
    [$\BigO(\#infoActualFanVivos(j) * tam(habitacion(j)) + \#infoActualPjVivos(j))$]
    ...
    [$O(\#infoActualFanVivos(j) * tam(habitacion(j)))$]
