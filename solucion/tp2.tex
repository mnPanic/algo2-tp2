...
\begin{algorithm}[H]{\textbf{iNuevoMapa}(\In{n}{nat}) $\to$ $res$ : $mapa$}
	\begin{algorithmic}[1]
      ...
      \While{$i < n$}
        \State $v.AgregarAtras(false)$            \Comment $O(1)$
        \State $i \gets i + 1$                    \Comment $O(1)$
      \EndWhile
      \State $i \gets 0$
      \While{$i < n$}                             \Comment $\Theta(n^2)$
        \State $res.AgregarAtras(v.Copiar())$     \Comment $\Theta(n)$
        \State $i \gets i + 1$                    \Comment $O(1)$
      \EndWhile
      \medskip
      \Statex \underline{Complejidad:} $\Theta(n^2)$
      \Statex \underline{Justificación:} Copiar un booleano es $\Theta(1)$, un vector de n bools posiciones $\Theta(n)$, entonces agregar n veces la copia del vector es $\Theta(n^2))$
    	\end{algorithmic}
\end{algorithm}
