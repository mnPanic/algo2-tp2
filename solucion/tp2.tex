...
\section{Módulo Juego}
  ...
  \begin{Interfaz}
    ...
    [$\BigO(1)$]
    [conocer la posicion y direccion actual del fantasma especial]

    \InterfazFuncion{infoActualFanVivosQueDisp}{\In{j}{Juego}}{Tupla(pos, dir)}%
    {res $\igobs$ infoActualFanVivosQueDisp(j)}%
    [$\BigO(\#fv)$]
    [conocer la posicion y direccion actual de los fantasmas vivos que disparan en el paso actual]
    ...
    \InterfazFuncion{actualizarConAccionDePj}{\In{pj}{string}, /In{a}{acc}, \Inout{j}{juego}}{}
    [$j \igobs j_0$]
    ...
    \begin{tad}{Juego Extendido($\alpha$)}
      ...
      \tadAxioma{infoActualFanVivosQueDispAux($j, fs$)}{\IF fantasmaVivo(dameuno(fs), j) $\land$ $\Pi_3(ult(dameuno(fs))$  
