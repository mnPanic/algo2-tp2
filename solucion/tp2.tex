...
\section{Módulo Juego}
  ...
  \begin{algorithm}[H]{\textbf{iIniciar}(\In{m}{mapa}, \In{pjs}{conj(jugador)}, \In{eventosFan}{vector(evento)}) $\to$ $res$ : estr}
        ...
        \State $iNuevoFan(res,\ eventosFan)$ \Comment $\Theta(long(eventosFan))$
        \medskip
  ...
  \begin{algorithm}[H]{\textbf{iNuevoFan}(\Inout{j}{estr}, \In{eventosFan}{vector(evento)})} \Comment Función privada
    \begin{algorithmic}[1]
