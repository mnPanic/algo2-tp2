...
\section{Módulo Juego}
  ...
  \begin{Representacion}
    ...
    \Rep[estr][e]{\\
      ...
      $(\forall\ p: pos)(p \in e.disparosFanUltimoPaso \impluego$ son válidas y no están ocupadas en e.mapa) $\land$ \\
      No existen disparos de jugadores o fantasmas en e.mapaDisparos tal que alrededor de ese disparo no haya otros disparos o estén libres en e.mapa las celdas a su alrededor $\land$ \\ \\
      Una posición en e.mapaDisparos tiene un disparo de un personaje $\iff$ un personaje disparó en el paso que figura en la posición y no hubo disparos posteriores a ese paso efectuados por personajes. $\land$ \\ \\
      Una posición en e.mapaDisparos tiene un disparo de un fantasma $\iff$ un fantasma disparó en el paso que figura en la posición y no hubo disparos posteriores a ese paso efectuados por fantasmas. $\land$ \\ \\
      Los disparos son efectuados en línea recta desde la posición consecutiva a del evento, en la dirección del evento, hasta una posición ocupada o inválida. $\land$ \\ //Pueden sobreescribrise al efectuar un disparo en un paso posterior. \\ \\
      Una infoPJ  se corresponde con su infoActual $\iff$ (está vivo $\land$  la posición y la dirección son iguales a las de su último evento efectuado $\land$ su identidad es la clave que obtiene dicha infoPJ en e.infoJugadores) $\land$ \\
  ...
  \begin{algorithm}[H]{\textbf{iIniciar}(\In{m}{mapa}, \In{pjs}{conj(jugador)}, \In{eventosFan}{vector(evento)}) $\to$ $res$ : estr}
        ...
        \State $iNuevoFanEspecial(res,\ eventosFan)$ \Comment $\Theta(long(eventosFan))$
        \medskip
  ...
  \begin{algorithm}[H]{\textbf{iReiniciarFantasmas}(\Inout{j}{estr}, \In{pjMatoFanEspecial}{infoPJ})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \EndWhile
      \\
      \State // Creo un nuevo fantasma (secu de eventos) conviertiendo la lista enlazada de 
      \State // acciones del PJ que mató al fantasma especial en un vector.
      \State $nuevosEventosFan \gets Inversa(pjMatoFanEspecial.eventos)$
      \\
      \State // Lo agrego a las estructuras
      \State $iNuevoFanEspecial(j, nuevosEventosFan)$ \Comment $\Theta(long(nuevosEventosFan))$
      \medskip
