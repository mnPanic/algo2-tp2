...
\section{Módulo Juego}
  ...
  \begin{algorithm}[H]{\textbf{iIniciarJugadores}(\In{j}{estr}, \In{m}{mapa}, \In{pjs}{conj(jugador)})} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State \tab $infoActual \gets \tuple{identidad: pj,\ posicion: localizacion.pos,\ direccion: localizacion.dir}$
      ...
      \State \tab $infoPtr \gets \& Definir(j.infoJugadores,\ pj,\ info)$
      \\
  ...
  \begin{algorithm}[H]{\textbf{iAgregarDisparo}(\Inout{j}{estr}, \In{pos}{pos}, \In{dir}{dir}, \In{esFan}{bool})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \While{Valida?(j.mapa, posCopy) \yluego\ Libre(j.mapa, posCopy)} \Comment $\BigO(Tam(j.mapa))$
        ...
        \State \tab $\textbf{then}\ AgregarRapido(j.disparosFanUltimoPaso,\ posCopy)$ \Comment $\Theta(1)$
        \State \tab $\textbf{end if}$
        \State $\textbf{end if}$
        \\
  ...
  \begin{algorithm}[H]{\textbf{iNuevaRonda}(\In{j}{estr}, \In{pjMatoFanEspecial}{infoPJ})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State // Creo un nuevo fantasma (secu de eventos) con las acciones del PJ que mató al fantasma especial
