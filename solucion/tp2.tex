...
\section{Diccionario Trie ($\alpha$)}
  ...
  \begin{Interfaz}
    ...
    \textbf{géneros}: \TipoVariable{dicc$(string, \alpha)$}.

    \Titulo{Operaciones básicas de diccionario}

    \InterfazFuncion{Vacío}{}{dicc$(string, \alpha)$}%
    {$res$ $\igobs$ vacio}%
    [$\Theta(1)$]
    [genera un diccionario vacío.]

    \InterfazFuncion{Definir}{\Inout{d}{dicc($string$, $\alpha$)}, \In{k}{$string$}, \In{s}{$\alpha$}}{}
    [$d \igobs d_0$ $\land$ $\lnot$\ definido?($d$, $k$)]
    {$d$ $\igobs$ definir($d, k, s$)}
    [$\Theta(copy(string) + copy(s))$]
    [define la clave $k$ $\not\in$ claves($d$) con el significado $s$ en el diccionario.]
    [los elementos $k$ y $s$ se definen por copia.]

    \InterfazFuncion{Definido?}{\In{d}{dicc($string$, $\alpha$)}, \In{k}{$string$}}{bool}
    {$res$ $\igobs$ def?($d$, $k$)}
    [$\BigO(|k|)$]
    [devuelve \texttt{true} si y sólo $k$ está definido en el diccionario.]

    \InterfazFuncion{Significado}{\In{d}{dicc($string$, $\alpha$)}, \In{k}{$string$}}{$\sigma$}
    [def?($d$, $k$)]
  ...
  \begin{Representacion}
    ...
    \begin{Estructura}{dicc$(string, \alpha)$}[dic]
      ...
      \begin{Tupla}[nodo]
        ...
        \tupItemF{siguientes}{arreglo(puntero(nodo))[255]}
      \end{Tupla}
    ...
    \AbsFc[dicc]{dicc($string$, $\alpha$)}[d]{\IF vacía?($d$.claves) THEN vacío ELSE definir(prim($d$).claves, prim($d$).significado, Abs(fin($d$))) FI}
  \end{Representacion}

  \begin{Algoritmos}

    \begin{algorithm}[H]{\textbf{iVacía}() $\to$ $res$ : dicc($string$, $\alpha$)}
      \begin{algorithmic}[1]
           \State $res \gets \& \langle NULL, \ arreglo\_estático[255] \ de\ \alpha \rangle$ 				\Comment $\Theta(1)$
    ...
    \begin{algorithm}[H]{\textbf{iSignificado}(\Inout{d}{dicc($string$, $\alpha$)}, \In{k}{$string$})$\to$ $res$ : $\alpha$}
      \begin{algorithmic}[1]
        \State $Nodo \ actual \gets d.raiz$                                                      \Comment $\Theta(1)$

        \State $\textbf{for} \ (char \ c \ : \ k) \ \ \textbf{do}$                               \Comment $\BigO(|k|)$  
        \State \ \  $\textbf{if} \ (actual \DRef siguientes [toASCII(c)] \ \neq  \ NULL) $       \Comment $\Theta(1)$
        \State \ \ \ \ $\textbf{then} \ actual \gets (actual \DRef siguientes [toASCII(c)])$     \Comment $\Theta(1)$                                        \Comment $\Theta(1)$
        \State $\textbf{end for}$                                    
    ...
    \begin{algorithm}[H]{\textbf{iDefinido?}(\Inout{d}{dicc($string$, $\alpha$)}, \In{k}{$string$}) $\to$ $res$ : bool}
      \begin{algorithmic}[1]
           \State $Nodo \ actual \gets d.raiz$                                                      \Comment $\Theta(1)$

           \State $\textbf{for} \ (char \ c \ : \ k) \ \ \textbf{do}$                               \Comment $\BigO(|k|)$  
           \State \ \  $\textbf{if} \ (actual \DRef siguientes [toASCII(c)] \ \neq  \ NULL) $       \Comment $\Theta(1)$
           \State \ \ \ \ $\textbf{then} \ actual \gets (actual \DRef siguientes [toASCII(c)])$     \Comment $\Theta(1)$
           \State \ \ \ \ $\textbf{else} \ res \gets false$                                         \Comment $\Theta(1)$
           \State $\textbf{end for}$                                    
  ...
  \begin{Representacion}
    ...
    \begin{Estructura}{juego}[estr]
      \begin{Tupla}[j]
        ...
        \tupItem{jugadores}{diccTrie(string, itConjLineal(infoPJ))}\\ % iterador a infoJugadores
        \tupItem{infoJugadores}{conj(infoPJ)}\\
        \tupItem{infoActualJugadoresVivos}{conj(infoActualPJ)}\\
        \tupItem{jugadoresVivos}{conj(itConjLineal(infoPJ))}\\

        \Comentario{Fantasmas}
        \tupItem{infoFantasmas}{conj(infoFan)}\\
        \tupItem{infoActualFantasmasVivos}{conj(infoActualFan)}\\
        \tupItem{fantasmasVivos}{conj(itConjLineal(infoFan))}\\
        \tupItemF{fantasmaEspecial}{itConjLineal(infoActualFan)}
      \end{Tupla}
      ...
      \begin{Tupla}[infoPJ]
        ...
        \tupItemF{infoActual}{itConjLineal(infoActualPJ)}
      \end{Tupla}
