...
\section{Módulo Juego}
  ...
  \begin{Interfaz}
    ...
    {(\paratodo \ t  Tupla(jugador, pos, dir))((($\Pi_1(t) \in$ jugadores(j) 
    $\yluego$ jugadorVivo($\Pi_1(t)$, j)) $\yluego \Pi_2(t) \igobs$ posJugador($\Pi_1(t)$, j) $\land$ $\Pi_3(t) \igobs$ dirJugador($\Pi_1(t)$, j))
    $\impluego t \in$ res)}%
    [$\Theta(1)$]
    [conocer la identidad, posicion y direccion actual de los jugadores vivos]
    
    \InterfazFuncion{infoActualFanVivos}{\In{j}{Juego}}{conj(Tupla(pos, dir))}%
    {(\paratodo \ f fantasma)(f $\in$ fantasmas(j) $\yluego$ fantasmaVivo(f, j)
    $\impluego$ ($\exists$ \ t Tupla(pos, dir))($\Pi_1(ult(f)) \igobs Pi_1(t) \land $ $\Pi_2(ult(f)) \igobs \Pi_2(t) \yluego$
    t $\in$ res $\land$ noTieneRepetidos(res) ))}%
    [$\Theta(1)$]
    [conocer la posicion y direccion actual de los fantasmas vivos]

    \InterfazFuncion{infoActualFanEspecial}{\In{j}{Juego}}{Tupla(pos, dir)}%
    {($\exists$ \ f fantasma)(f $\in$ fantasmas(j) $\yluego$ fantasmaVivo(f, j)
    $\impluego$ ($\exists$ \ t Tupla(pos, dir))($Pi_1(ult(f)) \igobs Pi_1(t) \land $ $Pi_2(ult(f)) \igobs Pi_2(t) \yluego$
    t $\igobs$ res))}%
    [$\Theta(1)$]
    [conocer la posicion y direccion actual de los fantasmas vivos]
    ...
    \begin{tad}{Juego Extendido($\alpha$)}
      ...
      FI}
      \tadAxioma{infoActualFanEspecial($j$)}{$\tuple(posFantasma(fantasmaEspecial(j)), dirFantasma(fantasmaEspecial(j)))$}
  ...
  \begin{algorithm}[H]{\textbf{iIniciar}(\In{m}{mapa}, \In{pjs}{conj(jugador)}, \In{eventosFan}{vector(evento)}) $\to$ $res$ : estr}
        ...
        \State $iIniciarJugadores(res,\ m,\ pjs)$
        \\
        \State // Inicializo los fantasmas
        \State $iIniciarFantasmas(res,\ eventosFan)$
        \medskip
        \Statex \underline{Complejidad:} $\Theta(?)$
        \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iIniciarJugadores}(\In{j}{estr}, \In{m}{mapa}, \In{pjs}{conj(jugador)})} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State \tab $itInfoActual \gets AgregarRapido(j.infoActualJugadoresVivos,\ infoActual)$
      \\
      \State \tab // Creo la infoPJ con la actual
      \State \tab $info \gets iNuevaInfoPJ(localizacion,\ itInfoActual)$
      \State \tab // La agrego al trie y me guardo el puntero a la info guardada
      \State \tab $infoPtr \gets \& Definir(j.infoJugadores,\ j,\ info)$
      \\
      \State \tab // Agrego al conjunto de jugadores vivos el puntero a la info del PJ
      \State \tab $AgregarRapido(j.infoJugadoresVivos,\ infoPtr)$
      \State $\textbf{end for}$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(?)$
    \end{algorithmic}
  ...
  \begin{algorithm}[H]{\textbf{iNuevaInfoPJ}(\In{localizacion}{tupla(pos, dir), \In{itInfoActual}{itConj(infoActualPJ)}}) $\to res:$ infoPJ} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State $AgregarAtras(evts, evento)$           \Comment $\Theta(1)$
      \\
  ...
  \begin{algorithm}[H]{\textbf{iIniciarFantasmas}(\In{j}{estr}, \In{eventosFan}{vector(evento)})} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State $itInfoActualFan \gets AgregarRapido(infoActualFan, j.infoActualFantasmasVivos)$
      \\
      \State // Hago que el fantasma especial sea este
      \State $j.infoFantasmaEspecial \gets itInfoActualFan$
      \\
      ...
      \State $itInfoFan \gets AgregarRapido(infoFan,\ j.infoFantasmas)$
      \\
      \State // Agrego al conjunto de fantasmas vivos el interador a la info del Fan
      \State $AgregarRapido(itInfoFan,\ j.infoFantasmasVivos)$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(?)$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iPasarTiempo}(\Inout{j}{estr})}
    \begin{algorithmic}[1]
      ...
      \\
      \State // Actualizo el mapa de disparos con los fantasmas vivos
      \State $iActualizarMapaDisparosConFan(j)$   \Comment $\BigO(\#fv * m)$
      \\
  ...
  \begin{algorithm}[H]{\textbf{iActualizarMapaDisparosConFan}(\Inout{j}{estr})}
    \begin{algorithmic}[1]
      ...
      \\
      \State \tab // Obtengo el evento actual
      \State \tab $eventoActual \gets iEventoActualFan(infoFan, j.paso)$ \Comment $\Theta(1)$
      \\
      \State \tab // Si dispara, agrego su disparo a los del paso
      \State \tab $\textbf{if} \ (eventoActual.dispara?) $       
      \State \tab  $\textbf{then}\ iAgregarDisparoFan(j,\ infoActual.pos,\ infoActual.dir)$ \Comment $\BigO(m)$
      \State \tab $\textbf{end if}$
      \State $\textbf{end for}$
      \medskip
  ...
  \begin{algorithm}[H]{\textbf{iChequearMuerteJugadores}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State \tab  $\textbf{then}\ iMuerePJ(j, itPJVivos)$              \Comment $\Theta(1)$
      \State $\textbf{end if}$
      \\
  ...
  \begin{algorithm}[H]{\textbf{iAgregarDisparoFan}(\Inout{j}{estr}, \In{pos}{pos}, \In{dir}{dir})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Parado desde $pos$ en $mapaDisparos$, recorro hacia $dir$ 
      \State // hasta que me choco con un obstaculo o la pared.
      \While{Valida?(j.mapa, pos) \yluego\ Libre(j.mapa, pos)} \Comment $\BigO(Tam(j.mapa))$
        \State // Si no pasé ya por está posición con otro fantasma
        \State // (i.e si en el mapa de disparos no está ya este paso)
        \State $\textbf{if}\ mapaDisparos[pos.x][pos.y].pasoDispFan \neq j.paso$ \Comment $\Theta(1)$
        \State $\textbf{then}$
        \State \tab // Agrego la posición al conjunto de disparos y seteo el paso al actual 
        \State \tab $AgregarRapido(j.disparosUltimoPaso,\ pos)$ \Comment $\Theta(1)$
        \State \tab $mapaDisparos[pos.x][pos.y].pasoDispFan \gets j.paso$ \Comment $\Theta(1)$
        \State $\textbf{end if}$
        \\
        \State // Avanzo la posición en esa dirección
        \State $pos \gets Avanzar(pos, dir)$          \Comment $\Theta(1)$
      \EndWhile 
      \medskip
  ...
  \begin{algorithm}[H]{\textbf{iEventoActualFan}(\In{info}{infoFan}, \In{paso}{nat}) $\to res:$ evento} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State $idx \gets mod(j.paso, Longitud(info.eventos))$
      \State $res \gets info.eventos[idx]$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(1)$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iEventoActualPJ}(\In{info}{infoPJ}) $\to res:$ evento} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State $res \gets Ultimo(info.eventos)$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(1)$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iEjecutarAccion}(\In{j}{estr}, \In{a}{accion}, \In{pj}{jugador}) $\to res$ :estr}
    \begin{algorithmic}[1]
      ...
      \State $j.paso \gets j.paso + 1$            \Comment $\Theta(1)$
      \\
      \State // Obtengo la información del PJ
      \State $infoPJ \gets Obtener(j.infoJugadores, pj)$ \Comment $\BigO(|j|)$
      \\
      \State // Genero un evento con la acción
      ...
      \\
      \State // Actualizo el mapa de disparos con el jugador
      \State $iActualizarMapaDisparosConPJ(j, evtPJ)$ \Comment $\BigO(m)$
      \\
      \State // Veo que fantasmas mueren
      \State $iChequearMuerteFantasmas(j)$      \Comment $\Theta(\#fv)$
      \\
      \State // Si murió el fantasma especial, cambio de ronda
      \State if..
      \State $iNuevaRonda(j, infoPJ)$
      \State else,
      \State // Actualizo el mapa de disparos con los fantasmas vivos
      \State $iActualizarMapaDisparosConFan(j)$   \Comment $\BigO(\#fv * m)$
      \\
