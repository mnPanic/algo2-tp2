...
\begin{algorithm}[H]{\textbf{iTam}() $\to$ $res$ : nat}
    	\begin{algorithmic}[1]
...
\begin{algorithm}[H]{\textbf{iOcupar}(\Inout{m}{mapa}, \In{c}{tupla(int, int)})}
  \begin{algorithmic}
			...
			\medskip
			\Statex \underline{Complejidad:} $\Theta(1)$
      \Statex \underline{Justificación:} El acceso a una posición de un vector y su modificación es $\Theta(1)$ (TURBIO)
      %%%%%%%%%%%%% OJO, LA FUNCIÓN [] DEL MÓDULO VECTOR PERMITE MODIFICAR? (VECTOR ES ARRAY DE PUNTEROS) %%%%%%%%%%%%
    	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{iLibre}(\In{m}{mapa}, \In{c}{tupla(int, int)}) $\to$ $res$ : bool}
	\begin{algorithmic}[1]
			 \State $res \gets m[\Pi_1(c)][\Pi_2(c)]$	\Comment $\Theta(1)$
