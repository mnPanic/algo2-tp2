...
\section{Módulo Juego}
  ...
  \begin{algorithm}[H]{\textbf{iIniciar}(\In{m}{mapa}, \In{pjs}{conj(jugador)}, \In{eventosFan}{vector(evento)}) $\to$ $res$ : estr}
        ...
        \State iIniciarJugadores(res)
        \\
        \State // Inicializo los fantasmas
        \State iIniciarFantasmas(res)
        \medskip
        \Statex \underline{Complejidad:} $\Theta(?)$
        \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iIniciarJugadores}(\In{j}{estr})} \Comment Función privada
    \begin{algorithmic}[1]
  ...
  \begin{algorithm}[H]{\textbf{iIniciarFantasmas}(\In{j}{estr})} \Comment Función privada
    \begin{algorithmic}[1]
