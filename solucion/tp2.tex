...
\section{Módulo Juego}
  ...
  \begin{Interfaz}
    ...
    [$\neg vacio(pjs) \land (\forall e: evento) (esta?(e,\ eventosFan) \impluego e.pos \in libres(m))$]
    ...
    \InterfazFuncion{infoActualPjVivos}{\In{j}{Juego}}{conj(Tupla(jugador, pos, dir))}%
    ...
    \InterfazFuncion{infoActualFanVivos}{\In{j}{Juego}}{conj(Tupla(pos, dir))}%
    {$res \igobs infoActualFanVivos(j)$}%
    [$\BigO(1)$]
    [devuelve un conjunto referencias a la información de los fantasmas que están vivos.]
    [las referencias son no modificables.]

    \InterfazFuncion{infoActualFanEspecial}{\In{j}{Juego}}{Tupla(pos, dir)}%
    ...
    \InterfazFuncion{infoActualFanVivosQueDisp}{\In{j}{Juego}}{conj(Tupla(pos, dir))}%
    ...
    \InterfazFuncion{ejecutarAccion}{\Inout{j}{juego}, \In{a}{accion}, \In{pj}{jugador}}{}%
    ...
    \InterfazFuncion{pasarTiempo}{\Inout{j}{juego}}{}%
    {$j \igobs pasar(j_0)$}%
    [$\BigO(\#fv * m + \#jv)$]
    [actualiza sin acción de jugador.]

    \InterfazFuncion{posOcupadasPorDisparosFan}{\In{j}{juego}}{conj(posicion)}
  ...
  \begin{algorithm}[H]{\textbf{iIniciar}(\In{m}{mapa}, \In{pjs}{conj(jugador)}, \In{eventosFan}{vector(evento)}) $\to$ $res$ : estr}
        ...
        \State \tab $mapa: m,$                     \Comment $\Theta(copy(m))$
        \\
        ...
        \\
        \State \tab // Inicializo estructuras de jugadores y fantasmas como vacías
        ...
        \State $iIniciarJugadores(res,\ m,\ pjs)$ \Comment $\Theta(\#pjs*|k| + locJugadores)$
        \\
        \State // Inicializo los fantasmas
        \State $iIniciarFantasmas(res,\ eventosFan)$ \Comment $\Theta(long(eventosFan))$
        \medskip
  ...
  \begin{algorithm}[H]{\textbf{iIniciarJugadores}(\Inout{j}{estr}, \In{m}{mapa}, \In{pjs}{conj(jugador)})} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State $\textbf{for} \ (pj,\ localizacion : \ localPJs) \ \ \textbf{do}$                                            \Comment $\Theta(\#pjs*(|k|+copy(info)))$
      ...
      \medskip
      \Statex \underline{Complejidad:} $\Theta(?)$
      \Statex \underline{Justificación:} Copiar y generar iteradores, tuplas y conjuntos es $\Theta(1)$. Definir es $\Theta(|pj|)$ ya que copiar la tupla de info es $\Theta(1)$. Luego, definir \#pjs es $\Theta(\#pjs*|pj|)$. Finalmente, la complejidad de todo el algoritmo es $\Theta(\#pjs*|k| + locJugadores)$.
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iIniciarFantasmas}(\Inout{j}{estr}, \In{eventosFan}{vector(evento)})} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State $infoFan \gets \tuple{infoActual: itInfoActualFan,\ eventos: eventosFan}$ \Comment $\Theta(1)$
      ...
      \medskip
      \Statex \underline{Complejidad:} $\Theta(long(eventosFan))$
      \Statex \underline{Justificación:} Copiar y generar iteradores, tuplas y conjuntos es $\Theta(1)$. Copiar infoActual implica copiar un vector de eventos, que es $\Theta(long(eventosFan)*copy(evento)) = \Theta(long(eventosFan))$     
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iInfoActualFanVivosQueDisp}(\In{j}{estr}) $\to res:$ conj(infoActualFan)}
    \begin{algorithmic}[1]
      ...
      \State $\textbf{for} \ (infoFan : \ j.infoFantasmas) \ \ \textbf{do}$                         \Comment $\BigO(\#fv)$
      \State \tab $\textbf{if} \ (infoFan.vivo? \land \Pi_3(infoFan.eventos[mod(j.paso, Longitud(infoFan.eventos))]))$ 
      \State \ttab $\textbf{then} \ AgregarRapido(res, siguiente(infoFan.infoActual))$ \Comment $\Theta(copy(a))$ pero como infoFan.infoActual es una tupla, entonces $\Theta(1)$
      \State \tab $\textbf{end if}$
      \State $\textbf{end for}$
      \medskip
      \Statex \underline{Complejidad:} $\BigO(\#fv)$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iVivo?}(\In{j}{estr}, \In{pj}{string}) $\to res:$ bool}
    \begin{algorithmic}[1]
      \State $res \gets iSignificado(j.infoJugadores, pj).vivo?$ \Comment $\BigO(|pj|)$
      \medskip
  ...
  \begin{algorithm}[H]{\textbf{iPosOcupadasPorDisparosFan}(\In{j}{estr}) $\to res:$ conj(posicion)}
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Complejidad:} $\BigO(?)$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iFantasmas}(\In{j}{estr}) $\to res:$ conj(lista(evento))}
    \begin{algorithmic}[1]
      \State $\textbf{for} \ (infoFan : \ j.infoFantasmas) \ \ \textbf{do}$                         \Comment $\Theta(\#fantasmas)$
      \State \ttab $\textbf{then} \ AgregarRapido(res, infoFan.eventos)$                             \Comment $\Theta(infoFan.eventos)$ 
      \State $\textbf{end for}$
      \State $res \gets j.infoFantasma$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(?)$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iFantasmaEspecial}(\In{j}{estr}) $\to res:$ lista(evento)}
    \begin{algorithmic}[1]
      \State $\textbf{for} \ (infoFan : \ j.infoFantasmas) \ \ \textbf{do}$                         \Comment $\Theta(\#fantasmas)$
      \State \tab $\textbf{if} \ (infoFan.infoActual == j.infoFantasmaEspecial))$ 
      \State \ttab $\textbf{then} \ res \gets infoFan.eventos$ \Comment $\Theta(copy(infoFan.eventos))$ pero como infoFan.infoActual es una tupla, entonces $\Theta(1)$
      \State \tab $\textbf{end if}$
      \State $\textbf{end for}$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(\#fantasmas + copy(infoFan.eventos))$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iJugadores}(\In{j}{estr}) $\to res:$ conj(string)}
    \begin{algorithmic}[1]
      \State $res \gets iClaves(j.infoJugadores)$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(1)$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iAcciones}(\In{pj}{string}, \In{j}{estr}) $\to res:$ lista(evento)}
    \begin{algorithmic}[1]
      \State $res \gets iSignificado(j.infoJugadores, pj).eventos$          \Comment $\Theta(|pj| + copy(iSignificado(j.infoJugadores, pj).eventos))$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(|pj| + copy(iSignificado(j.infoJugadores, pj).eventos))$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iNuevaRonda}(\In{j}{estr}, \In{pjMatoFanEspecial}{infoPJ})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \\
      \State //// Reinicio Fantasmas
      \State // Creo un nuevo fantasma (secu de eventos) conviertiendo la lista enlazada de acciones del PJ que mató al fantasma especial en un vector.
      \State // Hago que ese fantasma sea el nuevo especial
      \\
      \State // Recorro $infoFantasmas$, por cada fantasma
      \State \tab // Obtengo $infoFan$
      \State \tab // Lo seteo como vivo
      \\
      \State \tab // Creo su $infoActual$ y la agrego a $infoActualFantasmasVivos$, guardandome su iterador
      \State \tab // Se que $infoActual$ será eventualmente consistente al realizarse un paso
      \\
      \State \tab // Le seteo el iterador a $infoFan$
      \\
      \State \tab // Agrego un iterador a su info a $infoFantasmasVivos$
      \\
      \State //// Reinicio Jugadores
      \State // Obtengo sus localizaciones
      \State // Por cada clave y valor de las localizaciones,
      %%%%Che, con Gasti observamos que cuando obtenes el significado del Trie no te devuelve una referneica modifciable.
      %%%% O bien lo ponemos en los aspectos de aliasing que es una referencia modificable, o podés volver a definir los elementos
      %%%% sobreeescribiendo los que estaban (con definir) como hacías cuando iniciabas el juego con iIniciarJugadores.
      \State \tab // Obtengo $infoPJ$ del trie
      \State \tab // Reinicio los eventos
      \State \tab // Agrego un primer evento con la localización obtenida
      \State \tab // Lo seteo como vivo
      \\g
  ...
  \begin{algorithm}[H]{\textbf{iActualizarPJ}(\In{pj}{jugador}, \In{a}{accion}) $\to res:$ infoPJ} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State $infoPJ \gets Obtener(j.infoJugadores,\ pj)$          \Comment $\BigO(|j|)$
      \\
...
\begin{Interfaz}
  ...
  \InterfazFuncion{inversa}{\Inout{es}{vector(evento)}}{}%
  {$res \igobs inversa(es)$}%
  [$\Theta(1)$]
 ...
 \Titulo{Algoritmos del módulo}
  ...
  \begin{algorithm}[H]{\textbf{iInversa}(\Inout{es}{vector(evento)})}
    \begin{algorithmic}[1]
      ...
      \State // es + nada + nada + nada + nada + nada + inversa(es)
      \\
      \State // Me guardo la long original ya que lo voy a modificar por referencia
      \State $longOriginal \gets Longitud(es)$                                                              \Comment $\Theta(1)$
      \\
      \State // Creo un evento que sea pasar y lo agrego 5 veces
      \State $eventoPasar \gets \tuple{pos: Ultimo(es).pos,\ dir: Ultimo(es).dir,\ disparo?: false} $       \Comment $\Theta(1)$
      \State $\textbf{for} \ (i = 0,\ ... \ , 4) \ \ \textbf{do}$                                           \Comment $\BigO(long(es)*5)$
      \State \tab $AgregarAtras(es,\ eventoPasar)$                                                          \Comment $\BigO(long(es))$
      \State $\textbf{end for}$
      \\
      ...
      \State \tab $AgregarAtras(es,\ invertir(es[i]))$                                                      \Comment $\BigO(long(es))$
      \State $\textbf{end for}$
      \medskip
  ...
  \begin{algorithm}[H]{\textbf{iAplicar}(\In{a}{acción}, \In{j}{juego}, \In{e}{evento}) $\to$ $res$ : evento}
        \begin{algorithmic}[1]
          ...
          \State \tab $\textbf{if} \ (a.dir = Arriba() \land \ \Pi_1(e.pos)+1 < Tam(j.mapa)\ \yluego$ \\ \tab $Libre(j.mapa,\ \tuple{ \Pi_1(e.pos)+1, \Pi_2(e.pos)}\ )$        \Comment $\Theta(1)$
          \State \ttab   $\textbf{then} \ res \gets \tuple{pos: \tuple{ \Pi_1(e.pos)+1, \Pi_2(e.pos)},\ dir: a.dir,\ disparo?: false}$       \Comment $\Theta(1)$
          \State \ttab  $\textbf{else} \ res \gets \tuple{pos: e.pos,\ dir: a.dir,\ disparo?: false}$ 
          \State \tab $\textbf{end if}$


          \State \tab $\textbf{if} \ (a.dir = Abajo() \land \ \Pi_1(e.pos)-1 < Tam(j.mapa)\ \yluego$ \\ \tab $Libre(j.mapa,\ \tuple{ \Pi_1(e.pos)-1, \Pi_2(e.pos)}\ )$        \Comment $\Theta(1)$
          \State \ttab   $\textbf{then} \ res \gets \tuple{pos: \tuple{ \Pi_1(e.pos)-1, \Pi_2(e.pos)},\ dir: a.dir,\ disparo?: false}$       \Comment $\Theta(1)$
          \State \ttab  $\textbf{else} \ res \gets \tuple{pos: e.pos,\ dir: a.dir,\ disparo?: false}$ 
          \State \tab $\textbf{end if}$


          \State \tab $\textbf{if} \ (a.dir = Derecha() \land \ \Pi_2(e.pos)+1 < Tam(j.mapa)\ \yluego$ \\ \tab $Libre(j.mapa,\ \tuple{ \Pi_1(e.pos), \Pi_2(e.pos)+1}\ )$        \Comment $\Theta(1)$
          \State \ttab   $\textbf{then} \ res \gets \tuple{pos: \tuple{ \Pi_1(e.pos), \Pi_2(e.pos)+1},\ dir: a.dir,\ disparo?: false}$       \Comment $\Theta(1)$
          \State \ttab  $\textbf{else} \ res \gets \tuple{pos: e.pos,\ dir: a.dir,\ disparo?: false}$ 
          \State \tab $\textbf{end if}$


          \State \tab $\textbf{if} \ (a.dir = Izquierda() \land \ \Pi_2(e.pos)-1 < Tam(j.mapa)\ \yluego$ \\ \tab $Libre(j.mapa,\ \tuple{ \Pi_1(e.pos), \Pi_2(e.pos)-1}\ )$        \Comment $\Theta(1)$
          \State \ttab   $\textbf{then} \ res \gets \tuple{pos: \tuple{ \Pi_1(e.pos), \Pi_2(e.pos)-1},\ dir: a.dir,\ disparo?: false}$       \Comment $\Theta(1)$
          \State \ttab  $\textbf{else} \ res \gets \tuple{pos: e.pos,\ dir: a.dir,\ disparo?: false}$ 
          \State \tab $\textbf{end if}$
