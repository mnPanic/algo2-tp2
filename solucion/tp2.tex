...
\section{Módulo Juego}
  ...
  \begin{algorithm}[H]{\textbf{iReiniciarDisparosFan}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State $j.disparosFanUltimoPaso \gets Vacio()$            \Comment $\BigO(Tam(j.disparosFanUltimoPaso))$
      \medskip
      \Statex \underline{Pre:} true
      ...
      Este arreglo en particular siempre tendrá longitud $\BigO(\#fv * m)$, ya que se llena con los disparos de los fantasmas que estén vivos en ese paso.
      Depende de que tan laxo uno sea con el manejo de memoria, podría llegar a tomarse como $\Theta(1)$.
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iReiniciarJugadores}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State $j.infoActualJugadoresVivos \gets Vacio()$ \Comment $\BigO(\#jv * |pjMasLargo|)$
      \State $j.infoJugadoresVivos \gets Vacio()$       \Comment $\BigO(\#jv * Longitud(eventoMasLargo))$
      \\
      ...
      \State \tab $infoPJ.eventos \gets Vacia()$    \Comment $\Theta(Longitud(infoPJ.eventos))$
      \\
