...
\section{Diccionario Trie ($\alpha$)}
  ...
  \begin{Algoritmos}
    ...
    \begin{algorithm}[H]{\textbf{iVacía}() $\to$ $res$ : diccTrie($string$, $\alpha$)}
      \begin{algorithmic}[1]
           \State // Le asigna un nuevo nodo a la raiz
           \State $res \gets nuevoNodo()$ 				\Comment $\Theta(1)$
    ...
    \begin{algorithm}[H]{\textbf{iSignificado}(\Inout{d}{diccTrie($string$, $\alpha$)}, \In{k}{$string$})$\to$ $res$ : $\alpha$}
      \begin{algorithmic}[1]
    ...
    \begin{algorithm}[H]{\textbf{iDefinido?}(\Inout{d}{diccTrie($string$, $\alpha$)}, \In{k}{$string$}) $\to$ $res$ : bool}
      \begin{algorithmic}[1]
           \State $Nodo \ actual \gets d.raiz$                                                      \Comment $\Theta(1)$
    ...
    \begin{algorithm}[H]{\textbf{iDefinir}(\Inout{d}{diccTrie($string$, $\alpha$)}, \In{k}{$string$}, \In{s}{$\alpha$})}
      \begin{algorithmic}[1]
    ...
    \begin{algorithm}[H]{\textbf{inuevoNodo}() $\to$ $res$ : diccTrie($string$, $\alpha$)} \Comment Función privada que crea un nuevo nodo
      \begin{algorithmic}[1]
           \State // Reserva la memoria para un nuevo nodo con significado null y siguientes vacíos
           \State $res \gets \& \langle significado: NULL, \ siguientes: arreglo\_estatico[256] \ de\ \alpha \rangle$ 				\Comment $\Theta(1)$
