...
\section{Diccionario Trie ($\alpha$)}
  ...
  \begin{Representacion}
    ...
    \Rep[diccTrie][d]{
      raiz no est� contenido en ninguno de sus siguientes ni sus siguientes
    }

    ~

    \Abs[estr]{diccTrie($string$, $\alpha$)}[e]{d}{
      - Para todo c string, def?(c,d) $\igobs$ estaDefinido(e) 
      ...
      \\ - 
      \IF vac�a?($d$.claves) THEN vac�o ELSE definir(prim($d$).claves, prim($d$).significado, Abs(fin($d$))) FI}
      \\ \\
  \end{Representacion}
  \begin{Algoritmos}
