...
\section{Módulo Juego}
  ...
  \begin{algorithm}[H]{\textbf{iPasarTiempo}(\In{j}{estr})}
    \begin{algorithmic}[1]
      ...
      \State $iActualizarMapaDisparos(j)$
      \\
      \State // Veo que jugadores mueren
      \State $iChequearMuerteJugadores(j)$
      // Por cada fantasma
      //  Si dispara
      //    Agregar disparo
  ...
  \begin{algorithm}[H]{\textbf{iActualizarMapaDisparos}(\In{j}{estr})}
    \begin{algorithmic}[1]
      \State // Recorro los fantasmas vivos
      \State $\textbf{for} \ (itInfoFan :\ j.infoFantasmasVivos) \ \ \textbf{do}$
      \State \tab // Obtengo la información del fantasma
      \State \tab $infoFan \gets Siguiente(itInfoFan)$
      \State \tab // Si dispara, agrego el disparo
      \State \tab $eventoActual \gets iEventoActualFan(infoFan,\ j.paso)$
      \State \tab $\textbf{if} \ (eventoActual.dispara?) $       
      \State \ttab  $\textbf{then}\ iAgregarDisparo(j,\ eventoActual)$
      \State $\textbf{end for}$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iEventoActualFan}(\In{info}{infoFan}, \In{paso}{nat}) $\to res:$ evento}
    \begin{algorithmic}[1]
...
\begin{Representacion}
  ...
  a.acción \igobs $"mover" \land Rep(a.dir)$
  %%Esto es medio bizarro, no? Qué deberíamos pedirle a la dirección cuando la acción es mover?%%
  }
  
  \mbox{}

  \Abs[acción]{acción}[a]{a$_{tad}$}{(a.acción \igobs $"disparar"$  $\land$ a$_{tad}$ \igobs $esDisparar(a_{tad}))$ $\lor$\\
  (a.acción \igobs $"pasar"$  $\land$ a$_{tad}$ \igobs $esPasar(a_{tad}))$ $\lor$\\
  (a.acción \igobs $"mover"$  $\land$ a$_{tad}$ \igobs $esMover(a_{tad}))$ $\yluego$ a.dir \igobs $direccion(a_{tad}))$
  }
...
ACA ACLARAR QUE NO NOS IMPORTA LA DIRECCION EN PASAR Y DISPARAR PERO NO PODEMOS DEJARLO VACÍO.
\medskip
 ...
 \Titulo{Algoritmos del módulo}
  ...
  \begin{algorithm}[H]{\textbf{iPasar}() $\to$ $res$ : acción}
        \begin{algorithmic}[1]
        \State // $""$ es el constructor por defecto del tipo que sea dirección en su Representación Interna"
        \State $res \gets \tuple{accion: "pasar",\ dir:  ""}$      \Comment $\Theta(1)$
        \medskip
        \Statex \underline{Complejidad:} $\Theta(1)$
        \end{algorithmic}
      \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iDisparar}() $\to$ $res$ : acción}
        \begin{algorithmic}[1]
        \State // $""$ es el constructor por defecto del tipo que sea dirección en su Representación Interna"
        \State $res \gets \tuple{accion: "disparar",\ dir:  ""}$      \Comment $\Theta(1)$
        \medskip
        \Statex \underline{Complejidad:} $\Theta(1)$
        \end{algorithmic}
      \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iInversa}(\Inout{es}{vector(evento)})}
    \begin{algorithmic}[1]
      ...
      \State $\textbf{for} \ (i = 0,\ ... \ , 5) \ \ \textbf{do}$                                          \Comment $\BigO(long(es)*5)$
      \State \tab $e \gets \tuple{pos: Ultimo(es).pos,\ dir: Ultimo(es).dir,\ disparo?: false} $                           \Comment $\Theta(1)$  
      \State \tab $AgregarAtras(es,\ e)$                                                                   \Comment $\BigO(long(es))$
      \State $\textbf{end for}$
      
      \State $\textbf{for} \ (i = long(es)-5,\ ... \ , 0) \ \ \textbf{do}$                                 \Comment $\BigO(long(es)^2)$
      \State \tab $e \gets invertir(es[i])$                                                                \Comment $\Theta(1)$  
      \State \tab $AgregarAtras(es,\ e)$                                                                   \Comment $\BigO(long(es))$
      \State $\textbf{end for}$
