...
\section{Módulo Juego}
  ...
  \begin{algorithm}[H]{\textbf{iActualizarFantasmas}(\Inout{j}{estr})}
    \begin{algorithmic}[1]
      ...
      \\
      \State \tab // Actualizo su información actual, obteniendo obtengo el evento actual
      ...
      \State \tab  $\textbf{then}\ iAgregarDisparoFan(j,\ eventoActual.pos,\ eventoActual.dir)$ \Comment $\BigO(m)$
      \State \tab $\textbf{end if}$
      \State $\textbf{end for}$
      \medskip
  ...
  \begin{algorithm}[H]{\textbf{iActualizarFan}(\In{info}{infoFan}, \In{paso}{nat}) $\to res:$ evento} \Comment Funcion privada
    \begin{algorithmic}[1]
