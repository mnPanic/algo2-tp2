...
\hypersetup{%
 ...
 pdfkeywords={TP2},
 pdftitle={TP2 Exorcismo Extremo},
 pdfsubject={TP2 Exorcismo Extremo}
}
...
\section{Introducción}
    ...
    \tab  \textbf{Vectorizar}: lista($\alpha$) $\rightarrow$ vector($\alpha$) \\
    \tab esta función puede ser fácilmente implementada iterando y tomando referencia de los elementos de la lista y \tab agregándolos atrás del vector.
    \\ \tab La complejidad de esta función será $\BigO(n)$ siempre y cuando $\alpha$ se copie en $\BigO(1)$.
...
\section{Módulo Juego}
  ...
  \begin{Interfaz}
    ...
    \TituloModulo{Operaciones básicas de Juego}
    ...
    \TituloModulo{Especificación de las operaciones auxiliares utilizadas en la interfaz}
    ...
    \begin{tad}{Juego Extendido($\alpha$)}
      ...
      \tadAxioma{infoActualPjVivosAux($j, js$)}{\textbf{if} $\emptyset?(js)$ \textbf{then} $\emptyset$ \textbf{else} \\
        \IF jugadorVivo(dameUno(js), j) THEN Ag($\tuple{posJugador(dameUno(js), j), dirJugador(dameUno(js),\ j)}$, \\infoActualPjVivosAux(j, sinUno(js))) 
      ELSE infoActualPjVivosAux(j, sinUno(js)) 
      FI \textbf{fi}\\}
      \tadAxioma{infoActualFanVivos($j$)}{infoActualPjVivosAux(j, jugadores(j)}
      \tadAxioma{infoActualFanVivosAux($j, fs$)}{ \textbf{if} $\emptyset?(fs)$ \textbf{then} $\emptyset$ \textbf{else} \\
      \IF fantasmaVivo(dameUno(fs), j) THEN Ag($\tuple{posFantasma(dameUno(fs), j), dirFantasma(dameUno(fs),\ j)}$, \\infoActualFanVivosAux(j, sinUno(fs))) 
      ...
      \tadAxioma{infoActualFanVivosQueDispAux($j, fs$)}{\textbf{if} $\emptyset?(fs)$ \textbf{then} $\emptyset$ \textbf{else} \\
      \IF fantasmaVivo(dameUno(fs), j) $\land$ disparando(dameUno(fs), step(j))  
      THEN Ag(dameUno(fs), infoActualFanVivosQueDispAux(j, sinUno(fs)))
      ELSE infoActualFanVivosQueDispAux(j, sinUno(fs)) FI \textbf{fi}\\}
  ...
  \begin{Representacion}
    ...
    \TituloModulo{Representación de Juego}
  ...
  \TituloModulo{Algoritmos del módulo}
      \medskip
    
  \begin{algorithm}[H]{\textbf{$\bullet$ iIniciar}(\In{m}{mapa}, \In{pjs}{conj(jugador)}, \In{eventosFan}{vector(evento)}) $\to$ $res$ : estr}
        \begin{algorithmic}[1]
  ...
  \begin{algorithm}[H]{\textbf{iIniciarJugadores}(\Inout{j}{estr}, \In{m}{mapa}, \In{pjs}{conj(jugador)})} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State \tab $infoActual \gets \tuple{identidad: pj,\ posicion: localizacion.pos,\ direccion: localizacion.dir}$     \Comment $\Theta(copy(pj)) = \Theta(|pj|)$
      ...
      \State \tab $AgregarRapido(j.infoJugadoresVivos,\ infoPtr)$                                                         \Comment $\Theta(1)$
      \State $\textbf{end for}$
      \medskip
      \Statex \underline{Pre:} pjs no es vacio
      \Statex \underline{Post:} se inicializan las estructuras de los jugadores
      \Statex \underline{Complejidad:} $\BigO(\#pjs*|pjMasLargo| + locJugadores)$
      \Statex \underline{Justificación:} Copiar y generar iteradores, tuplas y conjuntos es $\Theta(1)$. Definir es $\Theta(|pj|)$ ya que copiar la tupla de info es $\Theta(|pj|)$ (porque hay que copiar el nombre) y definir también. Luego, definir \#pjs es $\BigO(\#pjs*|pjMasLargo|)$. Finalmente, la complejidad de todo el algoritmo es $\BigO(\#pjs*|pjMasLargo| + locJugadores)$.
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iNuevaInfoPJ}(\In{localizacion}{tupla(pos, dir), \In{itInfoActual}{itConj(infoActualPJ)}}) $\to res:$ infoPJ} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State $\rangle$
      \medskip
      \Statex \underline{Pre:} el iterador es valido
      \Statex \underline{Post:} se genera la info pj con el iterador y los 
  ...
  \begin{algorithm}[H]{\textbf{iCrearEventosConLocalizacion}(\In{localizacion}{tupla(pos, dir))} $\to res:$ lista(evento)} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \State $res \gets evts$
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} se genera una lista con un solo evento que contiene la info de la localizacion
      \Statex \underline{Complejidad:} $\Theta(1)$
  ...
  \begin{algorithm}[H]{\textbf{iNuevoFanEspecial}(\Inout{j}{estr}, \In{eventosFan}{vector(evento)})} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} eventosFan no es vacío
      \Statex \underline{Post:} Se agrega un nuevo fantasma (el especial) a todas las estructuras de forma correcta
      \Statex \underline{Complejidad:} $\Theta(long(eventosFan)^2)$
      \Statex \underline{Justificación:} Copiar y generar iteradores, tuplas y conjuntos es $\Theta(1)$.
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{$\bullet$ \textbf{iInfoActualPjVivos}(\In{j}{estr}) $\to res:$ conj(infoActualPJ)}
    \begin{algorithmic}[1]
  ...
  \begin{algorithm}[H]{$\bullet$ \textbf{iInfoActualFanVivos}(\In{j}{estr}) $\to res:$ conj(infoActualFan)}
    \begin{algorithmic}[1]
  ...
  \begin{algorithm}[H]{$\bullet$ \textbf{iInfoActualFanEspecial}(\In{j}{estr}) $\to res:$ infoActualFan}
    \begin{algorithmic}[1]
  ...
  \begin{algorithm}[H]{$\bullet$ \textbf{iInfoActualFanVivosQueDisp}(\In{j}{estr}) $\to res:$ conj(infoActualFan)}
    \begin{algorithmic}[1]
      \State $fantasmasVivosQueDisp \gets Vacio()$        \Comment $\Theta(1)$
      \\
      ...
      \For{itInfoFan : \ j.infoFantasmasVivos}                 \Comment $\Theta(\#fv)$
        ...
        \\
        \State $\textbf{if} \ iEventoActualFan(infoFan, j.paso).dispara?$ 
  ...
  \begin{algorithm}[H]{$\bullet$ \textbf{iVivo?}(\In{j}{estr}, \In{pj}{string}) $\to res:$ bool}
    \begin{algorithmic}[1]
      \State $res \gets Significado(j.infoJugadores, pj).vivo?$ \Comment $\Theta(|pj|)$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(|pj|)$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{$\bullet$ \textbf{iEjecutarAccion}(\Inout{j}{estr}, \In{a}{accion}, \In{pj}{jugador}) $\to res$ :estr}
    \begin{algorithmic}[1]
      ...
      \State $infoPJ \gets iActualizarPJ(pj,\ a)$                       \Comment $\Theta(|pj|)$
      \State $evtPJ \gets iEventoActualPJ(infoPJ)$                      \Comment $\Theta(1)$
      \\
      \State // Reinicio los disparos de fantasmas
      \State $iReiniciarDisparosFan(j)$                             \Comment $\BigO(1)$
      \\
      ...
      \State \tab $iNuevaRonda(j,\ infoPJ)$  \Comment $\BigO(m^2 + \#f + locJugadores + \#j * (|pjMasLargo| + Longitud(evMasLarg)))$
      \State $\textbf{else}$
      ...
      \State $\textbf{end if}$
      \medskip
      \Statex \underline{Complejidad sin cambiar:} $\BigO(|pj| + m + \#fv + \#fv * m + \#jv) \in \BigO(|pj| + \#fv * m + \#jv)$
      \Statex \underline{Complejidad cambiando:} $\BigO(|pj| + m^2 + \#f + locJugadores + \#j * (|pjMasLargo| + Longitud(eventoMasLargo)))$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iActualizarPJ}(\In{pj}{jugador}, \In{a}{accion}) $\to res:$ infoPJ} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Busco la información del PJ enviando infoJugadores como referencia modificable
      \State $infoPJ \gets Significado(j.infoJugadores,\ pj)$          \Comment $\Theta(|pj|)$
      \\
      ...
      \State $AgregarAtras(infoPJ.eventos,\ evtPJ)$                \Comment $\Theta(copy(evtPJ))$
      \\
      ...
      \medskip
      \Statex \underline{Pre:} $pj$ está definido en $j.infoJugadores$
      \Statex \underline{Post:} Se actualiza la info del personaje y se devuelve una referencia a ella
      \Statex \underline{Complejidad:} $\Theta(|pj|)$
      \Statex \underline{Justificacion:} Copiar tuplas, naturales y operaciones del iterador son $\Theta(1)$.
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iEventoActualPJ}(\In{info}{infoPJ}) $\to res:$ evento} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} Se devuelve el último evento de $info.eventos$
      \Statex \underline{Complejidad:} $\Theta(1)$
  ...
  \begin{algorithm}[H]{\textbf{iReiniciarDisparosFan}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State $j.disparosFanUltimoPaso \gets Vacio()$            \Comment $\BigO(1)$
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} La lista de disparos de fantasmas es vacía
      \Statex \underline{Complejidad:} $\BigO(1)$
      \Statex \underline{Justificación:} Liberar la memoria de un conjunto de tuplas de naturales es $\BigO(1)$.
  ...
  \begin{algorithm}[H]{\textbf{iActualizarMapaDisparosConPJ}(\Inout{j}{estr}, \In{evtPJ}{evento})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State $\textbf{end if}$
      \medskip
      \Statex \underline{Pre:} La posicion del evento es valida en el mapa del juego.
      \Statex \underline{Post:} Si el evento es disparar, se llena el mapa con los disparos correctos.
  ...
  \begin{algorithm}[H]{\textbf{iAgregarDisparo}(\Inout{j}{estr}, \In{pos}{pos}, \In{dir}{dir}, \In{esFan}{bool})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \EndWhile 
      \medskip
      \Statex \underline{Pre:} La pos es valida en el mapa del juego.
      \Statex \underline{Post:} Si el evento es disparar, se llena el mapa con los disparos correctos.
      \Statex \underline{Complejidad:} $\BigO(m)$
  ...
  \begin{algorithm}[H]{\textbf{iChequearMuerteFantasmas}(\Inout{j}{estr}) $\to res:$ bool} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State $itFanVivos \gets CrearIt(j.infoFantasmasVivos)$ 
      \\
      \While{HaySiguiente(itFanVivos)} \Comment $\Theta(\#fv)$
      ...
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} Mata a los fantasmas que están en posiciones en las que hay disparos en este paso
      \Statex \underline{Complejidad:} $\Theta(\#fv)$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iFanAfectadoPorDisparo}(\In{j}{estr}, \In{pos}{pos}) $\to res:$ bool} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} pos es valida en el mapa
      \Statex \underline{Post:} res es true sii en pos hay un disparo de un jugador en este paso
      \Statex \underline{Complejidad:} $\Theta(1)$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iMuereFan}(\Inout{j}{estr}, \Inout{itFanVivos}{itConj(itConj(infoFan))}) $\to res:$ bool} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} el iterador es valido
      \Statex \underline{Post:} Saca al fantasma de las estructuras que solo tienen vivos
      \Statex \underline{Complejidad:} $\Theta(1)$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iNuevaRonda}(\Inout{j}{estr}, \In{pjMatoFanEspecial}{infoPJ})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State $iReiniciarFantasmas(j)$  \Comment $\Theta(\#f)$
      \\
      \State // Agrego el nuevo fan especial luego de convertir la lista en un vector
      \State $nuevoFan \gets Vectorizar(pjmatoFanEspecial.eventos)$
      \State $iNuevoFanEspecial(j, nuevoFan)$ \Comment $\Theta(long(nuevoFan))$
      \\
      \State // Reinicio los jugadores
      \State $iReiniciarJugadores(j)$ \Comment $\BigO(locJugadores + \#j * (|pjMasLargo| + Longitud(eventoMasLargo)))$
      \medskip
      \Statex \underline{Pre:} el pj es uno de los del juego
      \Statex \underline{Post:} se agrega un nuevo fantasma especial correspondiente al jugador que lo mató. $\land$ Se reinician todas las estructuras
      \Statex \underline{Complejidad:} $\BigO(m^2 + \#f + locJugadores + \#j * (|pjMasLargo| + Longitud(eventoMasLargo)))$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iReiniciarMapaDisparos}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} el mapa de disparos no tiene mas disparos
      \Statex \underline{Complejidad:} $\Theta(m^2)$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iReiniciarFantasmas}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Vacío la información de los fantasmas vivos, liberando la memoria que ocupaba.
      \State $infoFantasmasVivos \gets Vacio()$         \Comment $\Theta(1)$
      \State $infoActualFantasmasVivos \gets Vacio()$   \Comment $\Theta(1)$
      \\
      \State // Recorro $infoFantasmas$ con un iterador
      \State $itInfoFan \gets CrearIt(j.infoFantasmas)$ \Comment $\Theta(1)$
      \While{HaySiguiente(itInfoFan)} \Comment $\Theta(\#f)$
        \State // Obtengo su info
        ...
        \State $itInfoActualFan \gets AgregarRapido(j.infoActualFantasmasVivos,\ infoActualFan)$             \Comment $\Theta(copy(infoActualFan))$
        \\
        ...
        \State $AgregarRapido(j.infoFantasmasVivos,\ itInfoFan)$       \Comment $\Theta(copy(itInfoFan)))$
      \EndWhile
      \\
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} todos los fantasmas están en las estructuras de vivos
      \Statex \underline{Complejidad:} $\Theta(\#f)$
      \Statex \underline{Justificación:} Copiar iteradores, tuplas, naturales, etc. es $\Theta(1)$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iReiniciarJugadores}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Vacío las estructuras de vivos
      \State $j.infoActualJugadoresVivos \gets Vacio()$ \Comment $\BigO(1)$
      \State $j.infoJugadoresVivos \gets Vacio()$       \Comment $\BigO(1)$
      \\
      ...
      \State $\textbf{for}\ pj, localizacion : localPJs\ \textbf{do}$ \Comment $\BigO(\#j * (|pjMasLargo| + Longitud(eventoMasLargo))$
      \State \tab // Obtengo $infoPJ$ del trie por referencia modificable (enviandole infoJugadores como modificable)
      \State \tab $infoPJ \gets Significado(j.infoJugadores, pj)$   \Comment $\Theta(|pj|)$
      \\
      \State \tab // Vacío los eventos, así liberando la memoria que ocupaban
      \State \tab $infoPJ.eventos \gets Vacia()$    \Comment $\Theta(1)$
      \\
      ...
      \State \tab $infoActualPJ \gets \tuple{identidad: pj, posicion: localizacion.pos, direccion: localizacion.dir}$ \Comment $\Theta(|pj|)$
      \\
      ...
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} todos los fantasmas están en las estructuras de vivos
      \Statex \underline{Complejidad:} $\BigO(locJugadores + \#j * (|pjMasLargo| + Longitud(eventoMasLargo)))$
      \Statex \underline{Justificacion:} Copiar tuplas, iteradores y direcciones es $\Theta(1)$
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iActualizarFantasmas}(\Inout{j}{estr})} \Comment Función privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} actualiza las acciones de los fantasmas, actualizando el mapa de disparos si disparan.
  ...
  \begin{algorithm}[H]{\textbf{iActualizarFan}(\Inout{info}{infoFan}, \In{paso}{nat}) $\to res:$ evento} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} actualiza la info actual del fantasma
      \Statex \underline{Complejidad:} $\Theta(1)$
  ...
  \begin{algorithm}[H]{\textbf{iEventoActualFan}(\In{info}{infoFan}, \In{paso}{nat}) $\to res:$ evento} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} devuelve el evento que corresponde al paso actual
      \Statex \underline{Complejidad:} $\Theta(1)$
      \Statex \underline{Justificación:} La operaciones matemáticas y la indexación en un vector es $\Theta(1)$.
    \end{algorithmic}
  \end{algorithm}
  ...
  \begin{algorithm}[H]{\textbf{iChequearMuerteJugadores}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} quita al jugador de las estr de vivos y lo deja como muerto si está en una pos en la que hay un disparo de un fan
  ...
  \begin{algorithm}[H]{\textbf{iPJAfectadoPorDisparo?}(\In{j}{estr}, \In{pos}{pos}) $\to res:$ bool} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \State $res \gets afectado?$
      \medskip
      \Statex \underline{Pre:} la pos es válida en el mapa del juego
      \Statex \underline{Post:} El PJ estará afectado si en la posición en la que está hay un disparo de un fantasma
      \Statex \underline{Complejidad:} $\Theta(1)$
      \Statex \underline{Justificacion:} La indexación en arreglos es $\Theta(1)$
   \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iMuerePJ}(\Inout{j}{estr}, \Inout{itPJVivos}{itConj(puntero(infoPJ))})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Obtengo una referencia modificable a su información
      ...
      \medskip
      \Statex \underline{Pre:} el iterador es válido
      \Statex \underline{Post:} modifica el booleano y quita su info de las estr de vivos
      \Statex \underline{Complejidad:} $\Theta(1)$
  ...
  \begin{algorithm}[H]{\textbf{iAgregarPasarFaltantes}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      ...
      \medskip
      \Statex \underline{Pre:} true
      \Statex \underline{Post:} agrega 'pasar' a los jugadores que no hayan realizado una acción
      \Statex \underline{Complejidad:} $\Theta(\#jv)$
      \Statex \underline{Justificacion:} Copiar un evento es $\Theta(1)$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{$\bullet$ \textbf{iPasarTiempo}(\Inout{j}{estr})}
    \begin{algorithmic}[1]
      \State // Incremento el paso
      \State $j.paso \gets j.paso + 1$            \Comment $\Theta(1)$
      \\
      ...
      \medskip
      \Statex \underline{Complejidad:} $\BigO(\#fv * m + \#jv)$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{$\bullet$ \textbf{iPosOcupadasPorDisparosFan}(\In{j}{estr}) $\to res:$ conj(posicion)}
    \begin{algorithmic}[1]
  ...
  \begin{algorithm}[H]{\textbf{iFantasmas}(\In{j}{estr}) $\to res:$ conj(vector(evento))}
    \begin{algorithmic}[1]
      \State $res \gets Vacio()$ \Comment $\Theta(1)$
      \State $\textbf{for} \ (infoFan : \ j.infoFantasmas) \ \ \textbf{do}$                         \Comment $\BigO(\#f * Longitud(eventoMasLargo))$
      \State \tab  $AgregarRapido(res, infoFan.eventos)$                             \Comment $\Theta(copy(infoFan.eventos)) = \Theta(Longitud(infoFan.eventos))$
      \State $\textbf{end for}$
      \medskip
      \Statex \underline{Complejidad:} $\BigO(\#f * Longitud(eventoMasLargo))$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iFantasmaEspecial}(\In{j}{estr}) $\to res:$ vector(evento)}
    \begin{algorithmic}[1]
      \State $\textbf{for} \ (infoFan : \ j.infoFantasmas) \ \ \textbf{do}$                         \Comment $\Theta(\#f)$
      \State \tab $\textbf{if} \ (infoFan.infoActual == j.infoFantasmaEspecial)$ 
  ...
  \begin{algorithm}[H]{\textbf{iJugadores}(\In{j}{estr}) $\to res:$ conj(string)}
    \begin{algorithmic}[1]
      \State $res \gets Claves(j.infoJugadores)$ \Comment $\Theta(1)$
      \medskip
      \Statex \underline{Complejidad:} $\Theta(1)$
    \end{algorithmic}
  \end{algorithm}
...
\begin{Interfaz}
  ...
  \TituloModulo{Operaciones básicas del mapa}
...
\begin{Representacion}
  ...
  \TituloModulo{Representación del mapa}
  \mbox{}
...
\begin{Interfaz}
  ...
  \TituloModulo{Operaciones básicas de Dirección}
...
\begin{Representacion}
  ...
  \TituloModulo{Representación de Dirección}
...
\begin{Interfaz}
  ...
  \TituloModulo{Operaciones básicas de Acción}
...
\begin{Representacion}
  ...
  \TituloModulo{Representación de Acción}
  \mbox{}
...
\section{Diccionario Trie ($\alpha$)}
  ...
  \begin{Interfaz}
    ...
    \TituloModulo{Operaciones básicas de diccionario}
  ...
  \begin{Representacion}
    ...
    \TituloModulo{Representación del diccionario}
...
\begin{Algoritmos}
    ...
    \begin{algorithm}[H]{\textbf{iDefinir}(\Inout{d}{dic}, \In{k}{$string$}, \In{s}{$\alpha$}) $\to$ $res$ : $\&\alpha$}
      ...
      \\  \State // Devuelvo por referencia el significado.
          ...
          \Statex \underline{Justificación:} Siempre se recorre toda la palabra para definirla, entonces el $for$ siempre tiene $|k|$ ciclos. La dereferenciación y comparación de punteros, e indexación en arreglos estáticos son $\Theta(1)$. Además $\Theta(|k| + copy(s) + copy(k))$ $\in$ $\Theta(|k| + copy(s))$. 
          \end{algorithmic}
    \end{algorithm}
