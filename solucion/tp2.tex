...
\section{Módulo Juego}
  ...
  \begin{algorithm}[H]{$\bullet$ \textbf{iEjecutarAccion}(\Inout{j}{estr}, \In{a}{accion}, \In{pj}{jugador}) $\to res$ :estr}
    \begin{algorithmic}[1]
      ...
      \State $\textbf{end if}$
      \medskip
      \Statex \underline{Complejidad:} Sin cambiar de ronda $\BigO(|pj| + m + \#fv + \#fv * m + \#jv) \in \BigO(|pj| + \#fv * m + \#jv)$, cambiando $\BigO(?)$
    \end{algorithmic}
  \end{algorithm}

  \begin{algorithm}[H]{\textbf{iActualizarPJ}(\In{pj}{jugador}, \In{a}{accion}) $\to res:$ infoPJ} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: $pj$ está definido en $j.infoJugadores$
      \State // Post: Se actualiza la info del personaje y se devuelve una referencia a ella
      \\
  ...
  \begin{algorithm}[H]{\textbf{iEventoActualPJ}(\In{info}{infoPJ}) $\to res:$ evento} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: Se devuelve el último evento de $info.eventos$
      \\
  ...
  \begin{algorithm}[H]{\textbf{iReiniciarDisparosFan}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: La lista de disparos de fantasmas es vacía
      \\
  ...
  \begin{algorithm}[H]{\textbf{iActualizarMapaDisparosConPJ}(\Inout{j}{estr}, \In{evtPJ}{evento})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: La posicion del evento es valida en el mapa del juego.
      \State // Post: Si el evento es disparar, se llena el mapa con los disparos correctos.
      \\
  ...
  \begin{algorithm}[H]{\textbf{iAgregarDisparo}(\Inout{j}{estr}, \In{pos}{pos}, \In{dir}{dir}, \In{esFan}{bool})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: La pos es valida en el mapa del juego.
      \State // Post: Si el evento es disparar, se llena el mapa con los disparos correctos.
      \\
  ...
  \begin{algorithm}[H]{\textbf{iChequearMuerteFantasmas}(\Inout{j}{estr}) $\to res:$ bool} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: Mata a los fantasmas que están en posiciones en las que hay disparos en este paso
      \\
  ...
  \begin{algorithm}[H]{\textbf{iFanAfectadoPorDisparo}(\In{j}{estr}, \In{pos}{pos}) $\to res:$ bool} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: pos es valida en el mapa
      \State // Post: res es true sii en pos hay un disparo de un jugador en este paso
      \\
  ...
  \begin{algorithm}[H]{\textbf{iMuereFan}(\Inout{j}{estr}, \Inout{itFanVivos}{itConj(itConj(infoFan))}) $\to res:$ bool} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: el iterador es valido
      \State // Post: Saca al fantasma de las estructuras que solo tienen vivos
      \\
  ...
  \begin{algorithm}[H]{\textbf{iNuevaRonda}(\Inout{j}{estr}, \In{pjMatoFanEspecial}{infoPJ})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: el pj es uno de los del juego
      \State // Post: se agrega un nuevo fantasma especial correspondiente al jugador que lo mató. $\land$
      \State // \tab\ \        Se reinician todas las estructuras
      \\
  ...
  \begin{algorithm}[H]{\textbf{iReiniciarMapaDisparos}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: el mapa de disparos no tiene mas disparos
      \\
  ...
  \begin{algorithm}[H]{\textbf{iReiniciarFantasmas}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: todos los fantasmas están en las estructuras de vivos
      \\
  ...
  \begin{algorithm}[H]{\textbf{iActualizarFantasmas}(\Inout{j}{estr})} \Comment Función privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: actualiza las acciones de los fantasmas, actualizando el mapa de disparos si disparan.
      \\
  ...
  \begin{algorithm}[H]{\textbf{iActualizarFan}(\Inout{info}{infoFan}, \In{paso}{nat}) $\to res:$ evento} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: actualiza la info actual del fantasma
      \\
  ...
  \begin{algorithm}[H]{\textbf{iEventoActualFan}(\In{info}{infoFan}, \In{paso}{nat}) $\to res:$ evento} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: devuelve el evento que corresponde al paso actual
      \\
  ...
  \begin{algorithm}[H]{\textbf{iChequearMuerteJugadores}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: quita al jugador de las estr de vivos y lo deja como muerto si está en una pos en la que hay un disparo de un fan
      \\
  ...
  \begin{algorithm}[H]{\textbf{iPJAfectadoPorDisparo?}(\In{j}{estr}, \In{pos}{pos}) $\to res:$ bool} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: la pos es válida en el mapa del juego
      \State // Post: El PJ estará afectado si en la posición en la que está hay un disparo de un fantasma
      \\
  ...
  \begin{algorithm}[H]{\textbf{iMuerePJ}(\Inout{j}{estr}, \Inout{itPJVivos}{itConj(puntero(infoPJ))})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: el iterador es válido
      \State // Post: modifica el booleano y quita su info de las estr de vivos
      \\
  ...
  \begin{algorithm}[H]{\textbf{iAgregarPasarFaltantes}(\Inout{j}{estr})} \Comment Funcion privada
    \begin{algorithmic}[1]
      \State // Pre: true
      \State // Post: agrega 'pasar' a los jugadores que no hayan realizado una acción
      \\
